import { $BN, $COL, $FD, $FM, $PT, $RP, $TG, Api, AppManager, DialogForm, Dialogs, Field, Master, Part } from 'vuetify-extended'
...
function servicePath() {
  ...
  return `rental-providers/${rentalProviderId}/settlement-batches`
}
  ...
  return `
    <div style="font-family:inherit; color:#10263b; background:#f7fbff; border:1px solid #d9e5f0; border-radius:18px; padding:18px;">
      <div style="margin-bottom:16px;">
        ...
        <div style="margin-top:6px; font-size:22px; font-weight:800;">${escapeHtml(batch?.beneficiaryName || useAppStore().rentalProvider?.name || 'Rental Provider')}</div>
      </div>
...
function renderPaginationControls(pagination: ReturnType<typeof buildPaginationState>, flowType: BatchFlowType) {
  const token = `rental-batches-${flowType}`
  return `
        ...
        ${pagination.visibleLines.map((line: any) => `
          <div style="padding:16px 18px; border-bottom:1px solid #e5edf4;">
            <div style="display:flex; flex-wrap:wrap; justify-content:space-between; gap:12px; align-items:flex-start;">
              <div>
                <div style="font-size:16px; font-weight:800; color:#10263b;">${escapeHtml(line?.orderNumber || 'Unknown reservation')}</div>
                ...
                <div style="margin-top:8px; display:flex; flex-wrap:wrap; gap:8px;">
                  ...
                  ${renderStatusChip(`reservation ${line?.orderRestaurantStatus || 'n/a'}`)}
                </div>
              </div>
              <div style="flex:1 1 300px; min-width:0; display:grid; grid-template-columns:repeat(auto-fit, minmax(140px, 1fr)); gap:10px;">
                <div><div style="font-size:11px; color:#61768b; text-transform:uppercase;">Reservation Total</div><div style="font-weight:700;">${escapeHtml(money(line?.totalAmount, batch?.currency))}</div></div>
...
async function loadBatchDetail(report: any, flowType: BatchFlowType) {
  ...
  try {
    const detail = await Api.instance.service(servicePath()).get(batchId, buildBatchDetailQuery(flowType, itemsPerPage, pageNumber))
...
function initializeBatchPagination(flowType: BatchFlowType) {
  const state = batchStateByFlow[flowType]
  ...
  document.addEventListener('click', (event) => {
    ...
    const button = target?.closest?.(`[data-settlement-batch-pagination="rental-batches-${flowType}"]`) as HTMLElement | null
...
async function exportBatchCsv(master: any, flowType: BatchFlowType) {
  const batchId = master?.$get('id')
  if (!batchId) {
    Dialogs.$error('Open a batch before exporting.')
    return
  }

  const detailState = master?.$get('detailState', master?.$data || {}) || {}
  const total = Math.max(Number(detailState?.settlementTotal ?? 0), 1)
  const detail = await Api.instance.service(servicePath()).get(batchId, buildBatchDetailQuery(flowType, total, 1))
  const settlements = Array.isArray(detail?.settlements) ? detail.settlements : []
  const rows = settlements.map((item: any) => ({
    batchReference: detail?.payoutReference || '',
    flowType: detail?.flowType || flowType,
    ...
    netAmount: money(item?.netAmount, detail?.currency),
    outstandingPayable: money(item?.outstandingPayableAmount, detail?.currency),
    outstandingRemittance: money(item?.outstandingRemittanceAmount, detail?.currency),
    eligibleAt: dateTime(item?.eligibleAt),
    createdAt: dateTime(item?.placedAt || item?.createdAt),
  }))

  downloadCsv(
    `${flowType === 'remittance' ? 'rental-remittance-batch' : 'rental-settlement-batch'}-${detail?.currency || 'export'}.csv`,
    rows,
  )
}

function openAcknowledgementDialog(master: any, report: any, flowType: BatchFlowType) {
  const batchId = master?.$get('id')

  if (!batchId) {
    Dialogs.$error('Open a payout batch before acknowledging it.')
    return
  }

  const dialog = new DialogForm({}, {
    form: () => $FM({ title: 'Acknowledge Payout Receipt', width: 640 }, {
      children: () => [
        $PT({}, {
          children: () => [
            $FD({ label: 'Reference', storage: 'payoutReference', readonly: true }, {
              default() {
                const detailState = master?.$get('detailState', master?.$data || {}) || {}
                return detailState?.payoutReference || master?.$get('payoutReference') || ''
              },
            }),
            $FD({ label: 'Acknowledgement Note', type: 'textarea', storage: 'acknowledgementNote', cols: 12 }),
          ],
        }),
      ],
      saved: async (form) => {
        try {
          await Api.instance.service(`${servicePath()}/${batchId}/acknowledge`).patch('', {
            acknowledgementNote: form.$master?.$get('acknowledgementNote'),
          })
          Dialogs.$success('Payout receipt acknowledged successfully.')
          await loadBatchDetail(report, flowType)
          dialog.forceCancel()
        } catch (error: any) {
          Dialogs.$error(error.message)
        }
...
function trigger(flowType: BatchFlowType) {
  return $TG({
    ...
    selectFields: [
      ...
      'beneficiaryAcknowledgedByLabel',
      'createdAt',
    ],
    queryFields: ['status', 'payoutReference', 'payoutMethod'],
    headers: [
      { title: 'Batch', value: 'id' },
      { title: 'Status', value: 'status' },
      { title: 'Currency', value: 'currency' },
      { title: 'Settlements', value: 'settlementCount' },
      { title: 'Payable', value: 'totalOutstandingPayableAmountStr', align: 'end' },
      { title: 'Remittance', value: 'totalOutstandingRemittanceAmountStr', align: 'end' },
      { title: 'Net', value: 'netPayoutAmountStr', align: 'end' },
      { title: 'Initiated', value: 'initiatedAtStr' },
      { title: 'Completed', value: 'completedAtStr' },
      { title: 'Acknowledged', value: 'beneficiaryAcknowledgedAtStr' },
    ],
    query: {
      $sort: { createdAt: -1 },
      $filters: { flowType: { $in: [flowType] } },
    },
    width: 1600,
  }, {
    format(_trigger, items) {
      for (const item of items) {
        item.totalOutstandingPayableAmountStr = money(item.totalOutstandingPayableAmount, item.currency)
        item.totalOutstandingRemittanceAmountStr = money(item.totalOutstandingRemittanceAmount, item.currency)
        item.netPayoutAmountStr = money(item.netPayoutAmount, item.currency)
        item.initiatedAtStr = dateTime(item.initiatedAt)
        item.completedAtStr = dateTime(item.completedAt)
        item.beneficiaryAcknowledgedAtStr = dateTime(item.beneficiaryAcknowledgedAt)
      }
      return items
    },
  })
...
function report(flowType: BatchFlowType) {
  const title = flowTitle(flowType)
  const fields: (Field | Part)[] = [
    $FD({ label: 'Summary', storage: 'summaryView', type: 'htmlview', readonly: true, cols: 12, minHeight: 260 }, {
      default(field) {
    ...
    $FD({ label: 'Settlements', storage: 'settlementsView', type: 'htmlview', readonly: true, cols: 12, minHeight: 520 }, {
      default(field) {
  ...
  return $RP({ title: `${title} Item`, sideButtonWidth: 280 },{
    form: () => $FM({ title: `${title} Item` }, {
      children: () => [$PT({}, { children: () => fields })],
      access: rentalAccess('rental.finance.view'),
    }),
    sideButtons: (_props, _context, reportInstance) => {
      const buttons = [
        $BN({ text: 'Export CSV', color: 'secondary' }, {
          async onClicked() {
            await exportBatchCsv(reportInstance?.$master, flowType)
          },
        }),
      ]

      const detail = reportInstance?.$master?.$get('detailState', reportInstance?.$master?.$data || {}) || {}
      if (flowType === 'payout' && detail?.status === 'paid_out' && !detail?.beneficiaryAcknowledgedAt) {
        buttons.push(
          $BN({ text: 'Acknowledge Receipt', color: 'success' }, {
            async onClicked() {
              openAcknowledgementDialog(reportInstance.$master, reportInstance, flowType)
            },
          }),
        )
      }
    ...
    setup(reportInstance) {
      batchStateByFlow[flowType].activeReport = reportInstance
      initializeBatchPagination(flowType)
      const master = reportInstance.$master as Master | undefined
      if (!master) {
        return
      }
      master.$set('itemsPerPage', 5)
      master.$set('pageNumber', 1)
      void loadBatchDetail(reportInstance, flowType)
    },
    loaded(reportInstance) {
      batchStateByFlow[flowType].activeReport = reportInstance
      void loadBatchDetail(reportInstance, flowType)
    },
    access: rentalAccess('rental.finance.view'),
...
function createCollection(flowType: BatchFlowType) {
  return $COL({ objectType: servicePath() }, {
    trigger: () => trigger(flowType),
    report: () => report(flowType),
    access: rentalAccess('rental.finance.view'),
  })
}

export const rentalSettlementBatchesCollection = () => createCollection('payout')
export const rentalRemittanceBatchesCollection = () => createCollection('remittance')

export const rentalSettlementBatchesMenu = () => makeCollectionMenu({
  title: 'Settlement Batches',
  collection: rentalSettlementBatchesCollection,
  access: rentalAccess('rental.finance.view'),
})

export const rentalRemittanceBatchesMenu = () => makeCollectionMenu({
