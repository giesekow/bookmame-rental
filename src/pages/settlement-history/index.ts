...
function servicePath() {
  ...
  return `rental-providers/${rentalProviderId}/settlements`
}
...
function renderHistorySummary(item: any, flowType: HistoryFlowType) {
  ...
  const cards = [
    ['Reservation', item?.rentalReservation?.reservationNumber || 'n/a'],
    ['Currency', item?.currency || 'n/a'],
    ['Payment Method', label(item?.paymentMethod)],
    ['Payment Status', label(item?.rentalReservation?.paymentStatus)],
    ['Reservation Status', label(item?.rentalReservation?.reservationStatus)],
  ...
  return `
    <div style="font-family:inherit; color:#10263b; background:#f7fbff; border:1px solid #d9e5f0; border-radius:18px; padding:18px;">
      <div style="margin-bottom:16px;">
        ...
        <div style="margin-top:6px; font-size:22px; font-weight:800;">${escapeHtml(item?.rentalReservation?.reservationNumber || 'Reservation')}</div>
        <div style="margin-top:10px; display:flex; flex-wrap:wrap; gap:8px;">
          ${renderStatusChip(item?.status)}
          ${renderStatusChip(`payment ${item?.rentalReservation?.paymentStatus || 'n/a'}`)}
          ${renderStatusChip(`reservation ${item?.rentalReservation?.reservationStatus || 'n/a'}`)}
        </div>
      </div>
  ...
  const trigger = () => {
    const trg = $TG({
      ...
      selectFields: [
        ...
        'createdAt',
        'rentalReservation',
        'rentalReservation.reservationNumber',
        'rentalReservation.paymentStatus',
        'rentalReservation.reservationStatus',
      ],
      queryFields: [
        'status',
        'paymentMethod',
      ...
      topChildren: () => [
        ...
        $FD({ label: 'Reservation Date From', type: 'date', storage: 'orderPlacedAtFrom', md: 3, lg: 3 }),
        $FD({ label: 'Reservation Date To', type: 'date', storage: 'orderPlacedAtTo', md: 3, lg: 3 }),
      ...
      format(_trigger, items) {
        for (const item of items) {
          item.reservationNumber = item?.rentalReservation?.reservationNumber || ''
...
export const rentalSettlementHistoryCollection = () => createCollection('payout')
export const rentalRemittanceHistoryCollection = () => createCollection('remittance')

export const rentalSettlementHistoryMenu = () => makeCollectionMenu({
  title: 'Settlement History',
  collection: rentalSettlementHistoryCollection,
  access: rentalAccess('rental.finance.view'),
})

export const rentalRemittanceHistoryMenu = () => makeCollectionMenu({
